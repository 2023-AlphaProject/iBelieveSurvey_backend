...
class CartListAPIView(APIView):
    ...
    def post(self, request, survey_id):
        ...
        if survey.user != request.user:
            return Response({'error': 'You are not authorized to create a cart for this survey.'},
                            status=status.HTTP_403_FORBIDDEN)

        # 해당 Survey의 is_idle 필드 확인
        if not survey.is_idle:
            return Response({'error': 'This survey is not idle.'}, status=status.HTTP_400_BAD_REQUEST)
